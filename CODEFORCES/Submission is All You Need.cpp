...
// is the sum of all elements in T. For example, if T={0,1,1,3}, then sum(T)=0+1+1+3=5
// .

// mex(T)
// is the smallest non-negative integer not in T. For example, if T={0,1,1,3}, then mex(T)=2 because 2 is the smallest non-negative integer not present in T
...
// (i.e., S′ contains some of the elements currently in S), add sum(S′) to your score, and then remove S′ from S
// .
// Select a subset S′⊆S
// , add mex(S′) to your score, and then remove S′ from S
...
// , add mex(S′)=mex({0,1})=2 to your score, and then remove S′ from S. Currently, your score is 2 and S={1}
// .
// Select S′={1}
// , add sum(S′)=sum({1})=1 to your score, and then remove S′ from S. Currently, your score is 3 and S=∅
