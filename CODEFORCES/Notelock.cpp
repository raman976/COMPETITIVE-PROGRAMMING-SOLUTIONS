...
// to protect.
// Then for each i
// (1≤i≤n) in increasing order, Teto can set si to 0 if all the following are true:
...
// is not protected,
// the previous k−1
// elements do not contain 1. More formally, 1 does not occur in smax(1,i−k+1),…,si−1
...
// and k (2≤n≤1000; 2≤k≤n) — the length of s and k
...
// . Now Teto cannot change s1 because it is protected and cannot change s2 because s1=1

// . It can be proven this is optimal.

// For the second testcase, you can protect only the first element and have s=100001
// . Teto cannot change s1 because it is protected and she cannot change s6 because there is 1 in the previous k−1 elements (100001

// ).

// For the fourth testcase, you must protect s1,s3,s5,s7
// and have s=1010101. It can be shown that this is optimal. For example, if you did not protect s3, then Teto can change it to 0 (1010101
