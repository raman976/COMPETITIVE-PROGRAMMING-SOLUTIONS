...
// . String s consists of lowercase letters of the Latin alphabet and has a length between 1 and 26
...
#include <iostream>
#include <string>
using namespace std;bool isAlphabetical(const string &s) {
    int n = s.size();
    int pos = s.find('a');
    if (pos == string::npos) return false;    int left = pos - 1;
    int right = pos + 1;
    char expected = 'b';    while (left >= 0 || right < n) {
        if (right < n && s[right] == expected) {
            ++right;
        } else if (left >= 0 && s[left] == expected) {
            --left;
        } else {
            return false;
        }
