...
// and b of equal length are similar, if they have the same character in some position (there exists an integer i such that ai=bi
...
// and a binary string s consisting of 2n−1 characters. Let's denote s[l..r] as the contiguous substring of s starting with l-th character and ending with r-th character (in other words, s[l..r]=slsl+1sl+2…sr

// ).

// You have to construct a binary string w
// of length n which is similar to all of the following strings: s[1..n], s[2..n+1], s[3..n+2], ..., s[n..2n−1]
...
// of length 2n−1. Each character si

// is either 0 or 1.
// Output

// For each test case, print the corresponding binary string w
// of length n. If there are multiple such strings — print any of them. It can be shown that at least one string w
