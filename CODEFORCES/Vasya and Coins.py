...
coins of 1 burle and b coins of 2 burles. He does not yet know the total cost of all goods, so help him find out s (s>0

): the minimum positive integer amount of money he cannot pay without change or pay at all using only his coins.

For example, if a=1
and b=1 (he has one 1-burle coin and one 2
...
-burle coin,
he can pay 3
burle without change by paying with one 1-burle coin and one 2
-burle coin,
he cannot pay 4
...
and bi (0≤ai,bi≤108) — the number of 1-burle coins and 2
...
burle coins, and he can collect either any amount from 1 to 4, but 5
can't.
...
def f(cases):
    ...
    for a, b in cases:
        if a == 0:
            ...
            answer.append(a + 2 * b + 1)
    return answer
t = int(input())
cases = [tuple(map(int, input().split())) for z in range(t)]
for ans in f(cases):
    print(ans)