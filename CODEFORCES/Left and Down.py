...
# of an infinite grid. Misha wants to move it to the cell (0,0). To do this, he has fixed some integer k

# .

# Misha can perform the following operation: choose two integers dx
# and dy (both from 0 to k inclusive) and move the robot dx cells to the left (in the direction of decreasing x coordinate) and dy cells down (in the direction of decreasing y coordinate). In other words, move the robot from (x,y) to (x−dx,y−dy)
...
# can be applied once. The robot will immediately go to (0,0), and the cost of the operation will be 1

# .

# In the second test case, the operations: (1,1)
# , (0,1), and (1,1) can be applied. After the first operation, the robot will be at cell (1,2), after the second one — at (1,1), and after the third one — at (0,0). The cost of the first and second operations is 1, while the third is 0, as the pair (1,1)
