...
// Let x
// denote the string obtained by removing all characters of p from s, while preserving the order of the remaining characters. Then x must be a palindrome‡
...
// †
// A subsequence of a string s=s1s2…sn is a sequence p=si1si2…sik such that 1≤i1<i2<…<ik≤n
...
// ‡
// A string t=t1t2…tm is a palindrome if ti=tm−i+1 for all 1≤i≤m
...
// (0≤k≤n) — the length of the subsequence p
// .
// On the second line, print k
// distinct integers i1,i2,…,ik (1≤i1<i2<⋯<ik≤n) — the indices of the characters in s that form p (in order as they appear in s
...
// (indices 3, 4), resulting in x=110011

// , which is a palindrome.

// In the fifth test case, we remove p=01
// (indices 5, 6), resulting in x=1001
